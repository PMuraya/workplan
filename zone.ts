...
import {basic_value, mutall_error, fuel, cname, attribute, database}  from "../../../schema/v/code/schema.js";
import {exec} from "../../../schema/v/code/server.js";
import {io, io_type, anchor} from "../../../schema/v/code/io.js";
import { metadata } from "../../../schema/v/code/library.js";
import {alias} from "../../../schema/v/code/questionnaire.js";
...
export type options = {
    ...
    //
    //The entitity and column name to be associated wwith all cells of a homozone
    //and from which the schema.column is established
    subject:[ename:string, cname:string, alias?:alias],
    //
    //The css to which the undelying table is attached if available
    css:string, 
}
...
//The class root of the tabulation system
class tabulation extends view{
    //
    //A dbname in teh tabulation hierrchy
    public dbname?:string;

    // datanase in the tabulation hierarchy
    public dbase?:database;
    //
    //The table that implements the zone
    public table?:HTMLTableElement;
    //
    //The css which defiens where the table is atatched
    public css?:string;
    //
    constructor(public parent?:tabulation){super()}
    //
    //Get the datanase in the tabulation hierechy; it must exist
    async get_dbase():Promise<database>{
        //
        //Define a datanse holder
        let dbase:database|undefined;
        //
        //Return a datatabase if it exists somewhere in the hierarchy path
        dbase = this.search_dbase();
        if (dbase) return this.dbase=dbase;
        //
        //Construct dbase from first principles
        //
        //Search for a database name, if it exists somewhere in the hierarchy
        const dbname:string|undefined = this.search_dbname();
        //
        //Its an error if no dbname is found in the tabulation hierarchy
        if (!dbname) throw new mutall_error(`Unable to get dbname in the tabulation hierarchy`);
        //
        //Get the structure of the dbname
        const Idbase = await exec('database',[dbname],'export_structure',[]);
        //
        //Construct a database
        dbase = new database(Idbase);
        //
        //Save and retern the final result, (to avoid re-computing the datanase)
         return this.dbase = dbase;
    }
    //
    //Search for a database in the tabulation hierarchy
    search_dbase():database|undefined{
        //
        //Return the datanase if it is already defined
        if (this.dbase) return this.dbase;
        //
        //If we are not yet at the top of the hierarchy, re-execute this procedure
        //using the parent; otherwise return undefined
        return  this.parent ? this.parent.search_dbase():undefined;
        
    }

    //
    //Search for a dbname in the tabulation hierarchy
    search_dbname():string|undefined{
        //
        //Return the dbname if it is already defined
        if (this.dbname) return this.dbname;
        //
        //If we are not yet at the top of the hierarchy, re-execute this procedure
        //using the parent; otherwise return undefined
        return  this.parent ? this.parent.search_dbname():undefined;
    }
    ...
    get_table():HTMLTableElement{
        ...
        const table:HTMLTableElement|undefined = this.parent?.get_table();
        //
        if (!table) throw new mutall_error(`No HTML table found in the tabulation hierarchy`);
        //
...
export abstract class zone extends tabulation{
    //
    ...
    //zone and its relative position in the heterozone
    get id():string{
        //
        //Get the parent of this zone
        const parent: css|{index:coord<number>, zone:heterozone} = this.index;
        //
    ...
    create_table():HTMLTableElement{
        //
        //Get the css where to create table; if non is found in the options
        const css:string = this.css ?? 'body'; 
        //
    ...
    get_neighbor(dim:0|1, delta:1|-1):zone|undefined{
        ...
        const parent:css|{zone:heterozone, index:coord<number>} = this.index;
...
export class homozone extends zone{
    ...
    //cell structure has an initial value of {},designed to make it easier to
    ...
    constructor(public options:Partial<options>={}, parent?:heterozone){
        //
        super(parent);
    }
...
export class heterozone extends zone{
    ...
    constructor(public children:layout, parent?:tabulation, index?:index, public dbname?:string){
        //
        super(parent);
        //
    ...
    set_child_parent(){
        ...
        for(let r=0; r<layout.length; r++){
            ...
            for(let c=0; c<layout[r].length; c++){
                ...
                const index:{index:coord<number>, zone:heterozone} = {index:[r,c], zone:this};
                //
                //Set the child's parant
                this.children[r][c].index = index;
            }
        }
    }
...
//The index of an axis ia also known as a tick
type tick = {value:string, attribute?:attribute}
//
//The stayic version ofa tick
type static_tick = {value:string, subject?:subject}|string;
//
type subject = [ename:string, cname:string, alias?:alias];
//
//The axis of a homozone is a abulation component
export class axis extends tabulation{
    //
    //The indices of an axis
    public indices?:Array<tick>;
    //
    //The attribute to be associated with all the index values
    public attribute?:attribute;
    //
    constructor(
        public mother:homozone,
        public static_indices:Array<static_tick>, 
        public subject?:[ename:string, cname:string]){
        super();
    }
    //
    //During the show the axis inputs are compiled
    async show():Promise<void>{
        //
        //Compile the axis
        this.indices = await this.compile_indices();
    }

    //Comile the axis inputs
    async compile_indices():Promise<Array<tick>>{
        //
        //Compile teh axis-level subject if it is available. This peoduces an 
        //attribute to be assoiated with all the index values
        const attribute = await this.get_attribute(this.subject);
        //
        //Start with an empty index
        const indices:Array<tick> = [];
        //
        //Define teh index variable
        let index:tick;
        //
        //Compile all the sttaic indices
        for(const static_index of this.static_indices){
            //
            index = await this.compile_index(static_index, attribute);
            //
            indices.push(index)
        }
        return indices;
    ...
    //Compile a static index
    async compile_index(static_index:static_tick, attribute?:attribute):Promise<tick>{
        //
        //Destrucure the static index type
        const si:{value:string, subject?:subject}|string = static_index;
        //
        //Consider the current situation where the index is a string
        //The compiled index is {value}
        if (typeof si==='string' && !attribute) return {value:si};
        //
        //If the axis attribute is known then all the index values will be assumed to be
        //of the column type
        if (typeof si==='string' && this.attribute) return {value:si, attribute};
        //
        //The index is of type {value:string, subject?:[ename, cname]}
        //
        //If there is no subject then use the axis-level attribute 
        if (typeof si!=='string' && !si.subject) return {value:si.value, attribute};
        //
        //If there is a subject, get use it to define the attribute of the axis
        if (typeof si!=='string' && si.subject) return {value:si.value, attribute:await this.get_attribute(si.subject)};
        //
        //At this point im unable to compile axis
        throw new mutall_error(`Unable to compile axis`);
    }

    async get_attribute(subject?:subject):Promise<attribute|undefined>{
        //
        //If teh subject is not defined , then teh attribuet is undefined
        if (!subject) return undefined;
        //
        //Destructure subject
        const [ename, cname] = subject;
        //
        //Get teh underlying datanase
        const dbase:database  = await this.get_dbase();
        //
        //Get teh entiry name
        const attribute:attribute|undefined = dbase.entities[ename].columns[cname];
        //
        //Its an error if the attribute is not defined
        if (!attribute) throw new mutall_error(`Attribute ${ename}.${cname} not found`);
        //
        return attribute;
    }
