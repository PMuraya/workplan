...
import { mutall_error, database } from "../../../schema/v/code/schema.js";
...
class tabulation extends view {
    ...
    async get_dbase() {
        ...
        const dbname = this.search_dbname();
        //
        //Its an error if no dbname is found in the tabulation hierarchy
        if (!dbname)
            throw new mutall_error(`Unable to get dbname in the tabulation hierarchy`);
        //
    ...
    get_table() {
        ...
        //
        if (!table)
            throw new mutall_error(`No HTML table found in the tabulation hierarchy`);
        //
...
export class zone extends tabulation {
    //
...
export class heterozone extends zone {
    ...
    constructor(children, parent, index, dbname) {
        //
        super(parent);
    ...
    set_child_parent() {
        ...
        for (let r = 0; r < layout.length; r++) {
            ...
            for (let c = 0; c < layout[r].length; c++) {
                ...
                const index = { index: [r, c], zone: this };
                //
...
export class axis extends tabulation {
    ...
    attribute;
    //
    constructor(mother, static_indices, subject) {
        super();
        this.mother = mother;
    ...
    async get_attribute(subject) {
        ...
        //
        //Get teh entiry name
        const attribute = dbase.entities[ename].columns[cname];
        //
        //Its an error if the attribute is not defined
        if (!attribute)
            throw new mutall_error(`Attribute ${ename}.${cname} not found`);
        //
        return attribute;
    }
}
